...
// Font rendering include (tarea_texto)
...
// Shader para colocar una textura (tarea_imagen)
...
// (tarea_imagen)
GLuint textureInit1ID, textureInit2ID, textureActiveID, textureScreenID;
bool iniciaPartida = false, presionarOpcion = false;

// Modelo para el render del texto (tarea_texto)
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	// Inicialización de los shaders (tarea_imagen)
	...
	// Se inicializa el modelo de render text (tarea_texto)
	...
	// Carga de texturas (tarea_imagen)
...
bool processInput(bool continueApplication) {
	...
	if (!iniciaPartida){
		...
		if(textureActiveID == textureInit1ID && presionarEnter) {
			iniciaPartida = true;
			textureActiveID = textureScreenID;
		}
		else if (!presionarOpcion && glfwGetKey(window, GLFW_KEY_LEFT_CONTROL) == GLFW_PRESS){
			presionarOpcion = true;
			if (textureActiveID == textureInit1ID)
				textureActiveID = textureInit2ID;
			else if (textureActiveID == textureInit2ID)
				textureActiveID = textureInit1ID;
		}
...
void applicationLoop() {
	...
	while (psi) {
		...
		// ******** Render de imagen de frente *********** (tarea_imagen)
		if (!iniciaPartida){
		...
		// Renderizado del texto, siempre al final (tarea_texto)
